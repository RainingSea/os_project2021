...
import nachos.vm.TranslationEntryWithPid;
...
//                e.printStackTrace();
    ...
    public void writeRegister(int number, int value) {
        ...
        Lib.assertTrue(number >= 0 && number < numUserRegisters);
        // 满足了这一行语句
        // 寄存器号为36 value不等于0 value不等于2
        if (number == 36 && value != 0 && value != 2) {
            System.out.println("写给该寄存器值" + value);
        }
    ...
    private int translate(int vaddr, int size, boolean writing)
        ...
        //如果使用页表，则处理器将看到页表，此时translations为页表。，如果不使用页表，则处理器将不会知道页表，只能看到TLB，此时translations代表TLB
        if (!usingTLB) {
...
//    private Privilege privilege;
    ...
    public static void printTLB(){
        System.out.println();
        System.out.println("————TLB使用情况————");
        for(TranslationEntry tlb:translations){
            System.out.println("vpn: "+tlb.vpn+" ppn: "+tlb.ppn+" valid: "+tlb.valid+" used: "+tlb.used+" dirty: "+tlb.dirty+" readOnly: "+tlb.readOnly);
        }
        System.out.println();
    }
    ...
    private class Instruction {
        ...
        private void decode() {
            ...
            System.out.println("mips机器指令 "+operation);
            name = info.name;
            format = info.format;
            flags = info.flags;
